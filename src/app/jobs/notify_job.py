...
class NotifyJob(Job):
    ...
    def can_process(self, event) -> bool:
        return event.get('type', '') == 'notify' and event.get('media_id')

    def do_process(self, event, context: ScheduleContext) -> bool:
        media_id, post_id, metadata, new_media_id, filename, new_filename, content_metadata = self.get_event_fields(
            event, 'media_id', 'post_id', 'metadata', 'new_media_id', 'filename', 'new_filename', 'content_metadata')
        ...
        media_repository.set_media(
            MediaData(media_id=media_id, media_path=filename, new_media_path=new_filename))
