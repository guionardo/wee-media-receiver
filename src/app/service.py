...
class MediaReceiverService:
    ...
    def __init__(self, config: Config, s3_client: S3Client = None, video_worker: VideoWorker = None, schedule_worker: ScheduleWorker = None):
        self.config = config
        self.s3_client = s3_client or S3Client(config)
        self.video_worker = video_worker or VideoWorker(config)
