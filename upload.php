...
require_once(ABSPATH . 'includes/libs/Nude/nf.php');
...
try {
    ...
    switch ($_POST["type"]) {
        ...
        $interacao = strtotime(date("Y-m-d H:i:s"));
        ...
        if ($_POST["multiple"] == "true") {
            ...
            foreach ($files as $file) {
                ...
                if (!isset($file) || $file["error"] != UPLOAD_ERR_OK) {
                    if ($files_num > 1) {
                        ...
                        modal(MESSAGE, __("Erro no upload"), __("Deu algum erro no upload"));
                    }
                }
                ...
                if ($file["size"] > $max_allowed_size) {
                    if ($files_num > 1) {
                        ...
                        modal(MESSAGE, __("Erro no upload"), __("Arquivo muito grande"));
                    }
                }

                if ($file['type'] == 'image/webp') {
                    return_json(array('callback' => 'formato'));
                }
                ...
                try {
                    ...
                    // $score = $filter->GetScore($file["tmp_name"]);
                    // var_dump($score);
                    // if($score >= 100){
                    //     return_json(array('callback' => 'nudes'));
                    // }


                    if ($image->getWidth() > 700) {
                        if ($_POST['handle'] != 'picture-user' || $_POST['handle'] != 'picture-page' || $_POST['handle'] != 'picture-group' || $_POST['handle'] != 'cover-user' || $_POST['handle'] != 'cover-page' || $_POST['handle'] != 'cover-group') {
                            $image->resizeWidth(500);
                        } else {
                            $image->resizeWidth(700);
                        }
                    } elseif ($image->getWidth() > 600) {
                        if ($_POST['handle'] != 'picture-user' || $_POST['handle'] != 'picture-page' || $_POST['handle'] != 'picture-group' || $_POST['handle'] != 'cover-user' || $_POST['handle'] != 'cover-page' || $_POST['handle'] != 'cover-group') {
                            $image->resizeWidth(500);
                        } else {
                            $image->resizeWidth(600);
                        }
                    } else {
                        if ($_POST['handle'] != 'picture-user' || $_POST['handle'] != 'picture-page' || $_POST['handle'] != 'picture-group' || $_POST['handle'] != 'cover-user' || $_POST['handle'] != 'cover-page' || $_POST['handle'] != 'cover-group') {
                            $image->resizeWidth(400);
                        }
                    }
                } catch (Exception $e) {
                    if ($files_num > 1) {
                        ...
                        modal(MESSAGE, __("Erro no upload"), __("Desculpe, neste espaço só é permitido o envio de imagens PNG, JPG e GIF. Obrigado"));
                    }
                }
                ...
                if ($image->_img_type == "image/gif" && !in_array($_POST['handle'], ['cover-user', 'picture-user', 'cover-page', 'picture-page', 'cover-group', 'picture-group', 'cover-event'])) {
                    if (!@move_uploaded_file($file["tmp_name"], $path)) {
                        modal(MESSAGE, __("Erro no upload"), __("Desculpe, não deu para enviar"));
                    }
                } else {
                ...
                if ($system['s3_enabled']) {
                    linode_space_upload($path, $image_name);
                }
            ...
            if (!isset($_FILES["file"]) || $_FILES["file"]["error"] != UPLOAD_ERR_OK) {
                modal(MESSAGE, __("Upload Error"), __("Desculpe, algo deu errado?"));
            }

                // check file size
            if ($_FILES["file"]["size"] > $max_allowed_size) {
                modal(MESSAGE, __("Aconteceu um erro"), __("Arquivo muito grande"));
            }
                // init image & prepare image name & path
            ...
            $file_name_copy = $directory . $prefix . 'copy.' . $extension;
            $path_copy = ABSPATH . $system['uploads_directory'] . '/' . $image_name;

            // $comando = "/usr/bin/ffmpeg -i ".$_FILES['file']['tmp_name']." -b 500k  ".$path_copy."";
            // exec($comando);
            ...
            if ($_POST['handle'] == 'picture-user' || $_POST['handle'] == 'picture-page' || $_POST['handle'] == 'picture-group') {
                ...
                if ($image->getWidth() < 150 || $image->getHeight() < 150) {
                    modal(MESSAGE, __("Foto muito pequena"), __("Escolha uma imagem com pelo menos 150 pixels de largura e pelo menos 150 pixels de altura"));
                }
            } elseif ($_POST['handle'] == 'cover-user' || $_POST['handle'] == 'cover-page' || $_POST['handle'] == 'cover-group') {

                if ($image->getWidth() > 1000) {
                    $image->resizeWidth(900);
                }

                if ($image->getWidth() < 600 || $image->getHeight() < 150) {
                    modal(MESSAGE, __("Foto muito pequena"), __("Escolha uma imagem com pelo menos 600 pixels de largura e pelo menos 300 pixels de altura"));
                }
            }
            ...
            if ($image->_img_type == "image/gif" && !in_array($_POST['handle'], ['cover-user', 'picture-user', 'cover-page', 'picture-page', 'cover-group', 'picture-group', 'cover-event'])) {
                ...
                if ($image->save($path, $system['uploads_quality'])) {
                    $salvou = true;
                }
            }
            ...
            switch ($_POST['handle']) {
                ...
                $db->query(sprintf("INSERT INTO posts (user_id, user_type, post_type, time, privacy,interacao) VALUES (%s, 'user', 'profile_cover', %s, 'public', %s)", secure($user->_data['user_id'], 'int'), secure($date), secure($interacao, 'int'))) or _error(SQL_ERROR_THROWEN);
                ...
                if ($salvou) {
                    ...
                    $db->query(sprintf("INSERT INTO posts (posts.user_id, posts.user_type, posts.post_type, posts.time, posts.privacy,posts.interacao) VALUES (%s, 'user', 'profile_picture', %s, 'public', %s)", secure($user->_data['user_id'], 'int'), secure($date), secure($interacao, 'int'))) or _error(SQL_ERROR_THROWEN);
                    $post_id = $db->insert_id;
                    ...
                    if ($db->insert_id) {
                        ...
                        /* update user profile picture */
                        $db->query(sprintf("UPDATE users SET user_picture = %s WHERE user_id = %s", secure($image_name), secure($user->_data['user_id'], 'int'))) or _error(SQL_ERROR_THROWEN);
                    }
                ...
                if ($salvou) {
                    ...
                    $db->query(sprintf("INSERT INTO posts (posts.user_id, posts.user_type, posts.post_type, posts.time, posts.privacy) VALUES (%s, 'page', 'page_cover', %s, 'public')", secure($page['page_id'], 'int'), secure($date))) or _error(SQL_ERROR_THROWEN);
                    $post_id = $db->insert_id;
                    /* insert new cover photo to album */
        ...
        $file_name_copy = $directory . $prefix . 'copy.' . $extension;
        // $path_copy = ABSPATH . $system['uploads_directory'] . '/' . $file_name;

        // ffmpeg -i YOURVID.mkv -vf scale=-1:480 -vcodec mpeg4 -qscale 3 /path/to/new/output.mp4
        // $comando = "/usr/local/bin/ffmpeg -i ".$_FILES['file']['tmp_name']." -b 800k  ".$path_copy."";
        // $comando = "/usr/bin/ffmpeg -i ".$_FILES['file']['tmp_name']." -b 800k  ".$path_copy."";
        // $comando = "/usr/bin/ffmpeg -i ".$_FILES['file']['tmp_name']." -c:v libx264 -crf 0".$path_copy."";
        // exec($comando);
        ...
        if ($system['s3_enabled']) {
            linode_space_upload($_FILES['file']['tmp_name'], $file_name);
        }
        ...
        if ($extension == 'mp4') {
            $nome = str_replace('.mp4', '', $_FILES["file"]["name"]);
        }
        if ($extension == 'mov') {
            $nome = str_replace('.mov', '', $_FILES["file"]["name"]);
        }
        if ($extension == 'mp3') {
            $nome = str_replace('.mp3', '', $_FILES["file"]["name"]);
        }
        $videos = $path_nome;

        $path_nome = ABSPATH . $system['uploads_directory'] . '/' . $folder . '/' . date('Y') . '/' . date('m') . '/thumb_' . $prefix;
        $path_nome_return = $folder . '/' . date('Y') . '/' . date('m') . '/thumb_' . $prefix;
        ...
        $command = "/usr/bin/ffmpeg -ss 1 -i " . $_FILES['file']['tmp_name'] . " -vframes 1 -q:v 2 " . $path_nome . ".jpg";
        exec($command);

            // echo $path_nome_return . ".jpg";
        if ($system['s3_enabled']) {
            linode_space_upload($path_nome . ".jpg", $path_nome_return . ".jpg");
        }
        @unlink($path_nome . ".jpg");

                // return the file new name & exit
        return_json(array("file" => $file_name, "poster" => $path_nome_return));
        break;
        ...
        if (!$system['s3_enabled']) {
            ...
            /* Amazon S3 */
            if ($system['s3_enabled']) {
                linode_space_upload($path_copy, $file_name);
            }
        }
